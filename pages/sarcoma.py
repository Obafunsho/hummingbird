"""
hummingbird/pages/sarcoma.py — Sarcoma · Soft Tissue & Bone
...
html_path = _ROOT / "static" / "hummingbird_sarcoma.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_sarcoma.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_sarcoma.html. Tried: {_ROOT / 'static' / 'hummingbird_sarcoma.html'}")
    st.stop()
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_sarcoma", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_sarcoma", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Sarcoma", key="sw_cur_sarcoma", use_container_width=True, disabled=True)
with _sw_cols[4]:
    if st.button("\u2191 More \u2193", key="sw_more_sarcoma", use_container_width=True):
        pass
