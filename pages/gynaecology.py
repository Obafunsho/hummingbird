"""
hummingbird/pages/gynaecology.py — Gynaecology · Suspected Cancer
...
<div style="display:flex;align-items:center;justify-content:space-between;
  ...
  <div style="display:flex;align-items:baseline;gap:12px;">
    ...
    <span style="font-size:11px;color:#999;letter-spacing:.1em;text-transform:uppercase;">
      Gynaecology · Suspected Cancer</span>
  </div>
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_gynaecology", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_gynaecology", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Gynaecology", key="sw_cur_gynaecology", use_container_width=True, disabled=True)
render_more_popover("gynaecology", _sw_cols[4])
