"""
hummingbird/pages/head_neck.py — Head & Neck · Suspected Cancer
...
html_path = _ROOT / "static" / "hummingbird_head_neck.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_head_neck.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_head_neck.html. Tried: {_ROOT / 'static' / 'hummingbird_head_neck.html'}")
    st.stop()
...
<div style="display:flex;align-items:center;justify-content:space-between;
  ...
  <div style="display:flex;align-items:baseline;gap:12px;">
    ...
    <span style="font-size:11px;color:#999;letter-spacing:.1em;text-transform:uppercase;">
      Head & Neck · Suspected Cancer</span>
  </div>
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_head_neck", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_head_neck", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Head & Neck", key="sw_cur_head_neck", use_container_width=True, disabled=True)
with _sw_cols[4]:
    if st.button("\u2191 More \u2193", key="sw_more_head_neck", use_container_width=True):
        pass
