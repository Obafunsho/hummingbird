"""
hummingbird/pages/prostate.py — Prostate · Suspected Cancer
...
html_path = _ROOT / "static" / "hummingbird_prostate.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_prostate.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_prostate.html. Tried: {_ROOT / 'static' / 'hummingbird_prostate.html'}")
    st.stop()
...
<div style="display:flex;align-items:center;justify-content:space-between;
  ...
  <div style="display:flex;align-items:baseline;gap:12px;">
    ...
    <span style="font-size:11px;color:#999;letter-spacing:.1em;text-transform:uppercase;">
      Prostate · Suspected Cancer</span>
  </div>
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_prostate", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_prostate", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Prostate", key="sw_cur_prostate", use_container_width=True, disabled=True)
with _sw_cols[4]:
    if st.button("\u2191 More \u2193", key="sw_more_prostate", use_container_width=True):
        pass
