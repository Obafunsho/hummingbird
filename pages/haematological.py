"""
hummingbird/pages/haematological.py — Haematological · Suspected Cancer
...
html_path = _ROOT / "static" / "hummingbird_haematological.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_haematological.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_haematological.html. Tried: {_ROOT / 'static' / 'hummingbird_haematological.html'}")
    st.stop()
...
<div style="display:flex;align-items:center;justify-content:space-between;
  ...
  <div style="display:flex;align-items:baseline;gap:12px;">
    ...
    <span style="font-size:11px;color:#999;letter-spacing:.1em;text-transform:uppercase;">
      Haematological · Suspected Cancer</span>
  </div>
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_haematological", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_haematological", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Haematological", key="sw_cur_haematological", use_container_width=True, disabled=True)
with _sw_cols[4]:
    if st.button("\u2191 More \u2193", key="sw_more_haematological", use_container_width=True):
        pass
