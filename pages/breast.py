"""
hummingbird/pages/breast.py — Breast · Suspected Cancer
...
html_path = _ROOT / "static" / "hummingbird_breast.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_breast.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_breast.html. Tried: {_ROOT / 'static' / 'hummingbird_breast.html'}")
    st.stop()
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_breast", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_breast", use_container_width=True):
