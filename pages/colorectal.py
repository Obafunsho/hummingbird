...
with _sw_col4:
    if st.button("Breast", key="sw_breast_col", use_container_width=True):
...
with left_col:
    ...
    fit_opts = [v for v,_,_ in FIT_OPTIONS]
    fit_fmt  = {v: l for v,l,_ in FIT_OPTIONS}
    fit_sel = st.segmented_control(
        "FIT", fit_opts,
    ...
    if new_syms != st.session_state.selected_symptoms:
        st.session_state.selected_symptoms = new_syms
        st.session_state.result = None
        st.rerun()
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    if new_mods != st.session_state.selected_modifiers:
        st.session_state.selected_modifiers = new_mods
        st.session_state.result = None
        st.rerun()
