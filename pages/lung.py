"""
hummingbird/pages/lung.py — Lung · Suspected Cancer
...
html_path = _ROOT / "static" / "hummingbird_lung.html"

# Fallback: try relative to cwd if _ROOT resolution fails
if not html_path.exists():
    html_path = Path("static") / "hummingbird_lung.html"

if not html_path.exists():
    st.error(f"Could not locate hummingbird_lung.html. Tried: {_ROOT / 'static' / 'hummingbird_lung.html'}")
    st.stop()
...
with _sw_cols[1]:
    if st.button("Colorectal", key="sw_col_lung", use_container_width=True):
        st.switch_page("pages/colorectal.py")
with _sw_cols[2]:
    if st.button("Upper GI", key="sw_ugi_lung", use_container_width=True):
        st.switch_page("pages/upper_gi.py")
with _sw_cols[3]:
    st.button("Lung", key="sw_cur_lung", use_container_width=True, disabled=True)
with _sw_cols[4]:
    if st.button("\u2191 More \u2193", key="sw_more_lung", use_container_width=True):
        pass
